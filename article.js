...
if(!article){document.getElementById('articlePage').innerHTML='<div class="empty">找不到这篇短文。<a href="index.html">返回阅读首页</a></div>'}
 ...
 pauseButton.onclick=()=>{if(speechSynthesis.paused){speechSynthesis.resume();gapPaused=false}else if(speechSynthesis.speaking){speechSynthesis.pause()}else if(gapTimer){clearTimeout(gapTimer);gapTimer=null;gapPaused=true}controls(true)};
