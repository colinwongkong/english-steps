...
if(!article){document.getElementById('articlePage').innerHTML='<div class="empty">找不到这篇短文。<a href="index.html">返回阅读首页</a></div>'}
 ...
 let repeat=false,utterance=null;
 ...
 function syncSpeechControls(){const active='speechSynthesis'in window&&speechSynthesis.speaking;pauseButton.disabled=!active;pauseButton.textContent=active&&speechSynthesis.paused?'继续':'暂停';playButton.textContent=active?'重新播放':'播放朗读'}
 function speak(){if(!('speechSynthesis'in window)){toast('此浏览器暂不支持语音朗读');return}speechSynthesis.cancel();utterance=new SpeechSynthesisUtterance(article.text);utterance.lang='en-US';utterance.rate=Number(speed.value);utterance.onstart=syncSpeechControls;utterance.onend=()=>{if(repeat){setTimeout(speak,180)}else syncSpeechControls()};utterance.onerror=syncSpeechControls;speechSynthesis.speak(utterance);syncSpeechControls()}
 playButton.onclick=speak;
 pauseButton.onclick=()=>{if(speechSynthesis.paused)speechSynthesis.resume();else if(speechSynthesis.speaking)speechSynthesis.pause();syncSpeechControls()};
